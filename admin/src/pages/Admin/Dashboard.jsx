...
const Dashboard = () => {
    const {aToken, getDashData, cancleAppointment, dashData} = useContext(AdminContext);
                ...
                <div className='pt-4 border border-t-0'>
                    {
                        dashData.latestAppointments.map((item, index) => (
                            <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
                                ...
                                {
                                    item.cancelled
                                    ? <p className='text-gray-800 font-medium'>Cancelled</p>
                                    : <img onClick={() => cancleAppointment(item._id)} className='w-10 cursor-pointer' src={assets.cancel_icon} />
                                }
                            </div>
                        ))
                    }
