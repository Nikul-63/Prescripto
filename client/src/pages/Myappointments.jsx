...
const Myappointments = () => {
    ...
    const initPay = (order) => {
        const options = {
            ...
            desription : 'Appointment Payment', 
            order_id : order.id, 
            ...
            handler : async (response) => {
                ...
                try
                {
                    const cleanUrl = `${backendUrl.replace(/\/$/,'')}/api/user/verifyRazorpay`;

                    const { data } = await axios.post(cleanUrl, response, { headers : { token }});
                ...
                {appointments.map((item,index)=>(
                    <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                        ...
                        <div className='flex flex-col gap-2 justify-end'>
                            ...
                            {!item.cancelled && <button onClick={() => cancleAppointment(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300'>Cancel Appointment</button> }
